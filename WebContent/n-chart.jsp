...
<%
   ...
   if(control == 1){ //control 1이면 시가총액 순 차트 출력, 2면 거래량 순 차트 출력
	   stockList = stockdao.getTopMarketCap("Nasdaq", 151); //미리 150개의 주식 정보를 리스트로 받아둠 - 시가총액 순으로
   }else{		
	   stockList = stockdao.getTopVolumeCap("Nasdaq", 151); //미리 150개의 주식 정보를 리스트로 받아둠 - 거래량 순으로
   }
   StockBean stockB;
%>
<body>
